...
function nametag:render()
    ...
    for _, v in ipairs(getElementsWithinRange(x, y, z, 35, 'player', getElementInterior(localPlayer), getElementDimension(localPlayer))) do 
        ...
        local scx, scy, scz = getWorldFromScreenPosition(bonex, boney, bonez + 0.5)
        ...
        dxDrawText('#FFFFFF[#FFFFFF'..getElementData(v, 'id')..'#FFFFFF] #FFFFFF'..getPlayerName(v):gsub('#%x%x%x%x%x%x','')..'', fX, fY, fX, fY, tocolor(255, 255, 255, 255), nametag.scale, nametag.font, 'center', 'center', false, false, false, true, true)
...
addEventHandler('onClientRender', root, nametag.render)
